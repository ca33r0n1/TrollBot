...
public class RCon {
    ...
    public void AmendClaimBlocks(final String player, Integer amount) throws IOException, AuthenticationException {
        final StringBuilder sb = new StringBuilder();
        sb.append("acb");
        sb.append(' ').append(player);
        sb.append(' ').append(amount.toString());
        final String response = send(sb.toString());
        assert "".equals(response);
    }

    public void SendCommand(String... command) throws IOException, IncorrectRequestIdException {
        final StringBuilder sb = new StringBuilder();
        for (String s: command){
            sb.append(s);
            sb.append(' ');
        }
        final String response = send(sb.toString().trim());
        assert "".equals(response);
    }
